...
# ifdef __aarch64__
const unsigned int OPENSSL_armcap_P = ARMV8_AES |
                                      ARMV8_PMULL |
                                      ARMV8_SHA1 |
                                      ARMV8_SHA256;
#else
const unsigned int OPENSSL_armcap_P = 0;
#endif
...
UC_EXPORT_API void gcm_init_v8(u128 Htable[16], const u64 Xi[2]);
UC_EXPORT_API void gcm_gmult_v8(u64 Xi[2], const u128 Htable[16]);
UC_EXPORT_API void gcm_ghash_v8(u64 Xi[2], const u128 Htable[16], const u8 *inp,
                  size_t len);
#  endif
...
void CRYPTO_gcm128_init(GCM128_CONTEXT *ctx, void *key, block128_f block)
{
    ...
    /*memset(ctx, 0, sizeof(*ctx));*/		//using ENV_sec_memzero instead of memset
	ENV_sec_memzero(ctx, sizeof(*ctx));
    ctx->block = block;
    ctx->key = key;
                         ...
                         size_t len)
{
    ...
    if (len == 12) {
        ENV_sec_memcpy( ctx->Yi.c, 12, iv, 12);
        ctx->Yi.c[12] = 0;
        ctx->Yi.c[13] = 0;
        ctx->Yi.c[14] = 0;
        ctx->Yi.c[15] = 1;
...
# endif
    ...
    if (ctx->ares) {
        ...
        ENV_sec_memcpy( ctx->Xn, sizeof(ctx->Xi), ctx->Xi.c, sizeof(ctx->Xi));
        ctx->Xi.u[0] = 0;
        ctx->Xi.u[1] = 0;
        mres = sizeof(ctx->Xi);
#else
...
# endif
    ...
    if (mres) {
        ...
        /*memset(ctx->Xn + mres, 0, blocks - mres);*/		// using ENV_sec_memzero instead of memset
		ENV_sec_memzero(ctx->Xn + mres, blocks - mres);
        mres = blocks;
        if (mres == sizeof(ctx->Xn)) {
            GHASH(ctx, ctx->Xn, mres);
            mres = 0;
    ...
    bitlen.hi = alen;
    bitlen.lo = clen;
    ENV_sec_memcpy( ctx->Xn + mres, sizeof(bitlen), &bitlen, sizeof(bitlen));
    mres += sizeof(bitlen);
    GHASH(ctx, ctx->Xn, mres);
#else
    ctx->Xi.u[0] ^= alen;
...
void CRYPTO_gcm128_tag(GCM128_CONTEXT *ctx, unsigned char *tag, size_t len)
{
    ...
    size_t ln = (len <= sizeof(ctx->Xi.c)) ? len : sizeof(ctx->Xi.c);
