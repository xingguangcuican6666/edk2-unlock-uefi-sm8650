...
//
#define DEBUG_PROPERTY_DEBUG_ENABLED       0x01
#define DEBUG_PROPERTY_BREAKPOINT_ENABLED  0x10
#define DEBUG_PROPERTY_DEADLOOP_ENABLED    0x20

//
// Define the maximum debug and assert message length that this library supports
//
#define MAX_DEBUG_MESSAGE_LENGTH  0x100
STATIC EFI_SERIAL_IO_PROTOCOL *SerialIo;
...
)
{
  ...
  Status = SerialIo->Write (SerialIo, &NumberOfBytes, Buffer);
  if (EFI_ERROR (Status)) {
    return 0;
  }
...
/**
  ...
  to the debug output device.  If DEBUG_PROPERTY_BREAKPOINT_ENABLED bit
  of PcdDebugProperyMask is set then CpuBreakpoint() is called. Otherwise, if
  DEBUG_PROPERTY_DEADLOOP_ENABLED bit of PcdDebugProperyMask is set then
  ...
  )
{
  ...
  if ((PcdGet8 (PcdDebugPropertyMask) &
      DEBUG_PROPERTY_BREAKPOINT_ENABLED) != 0) {
    CpuBreakpoint ();
  } else if ((PcdGet8 (PcdDebugPropertyMask) &
      DEBUG_PROPERTY_DEADLOOP_ENABLED) != 0) {
    CpuDeadLoop ();
  }
}
...
/**
  ...
  This function returns TRUE if the DEBUG_PROPERTY_DEBUG_ENABLED bit of
  PcdDebugProperyMask is set.  Otherwise FALSE is returned.

  @retval  TRUE    The DEBUG_PROPERTY_DEBUG_ENABLED bit of
            PcdDebugProperyMask is set.
  @retval  FALSE   The DEBUG_PROPERTY_DEBUG_ENABLED bit of
  ...
  )
{
  return (BOOLEAN) ((PcdGet8 (PcdDebugPropertyMask) &
          DEBUG_PROPERTY_DEBUG_ENABLED) != 0);
}
